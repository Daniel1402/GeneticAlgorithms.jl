...
function (c::RealUniformInitialization{T})()::Population{Chromosome{T}} where {T<:Float64}
    return Population([Chromosome(rand(Uniform(c.interval[1], c.interval[2]), c.chromosome_size)) for _ in 1:c.population_size])
end

function (c::RealUniformInitialization{T})()::Population{Chromosome{T}} where {T<:Integer}
    return Population([Chromosome(rand((c.interval[1], c.interval[2]), c.chromosome_size)) for _ in 1:c.population_size])
end
