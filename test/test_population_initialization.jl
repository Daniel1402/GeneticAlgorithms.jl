...
@testset "RealUniformInitialization" begin
    ...
    @test all([length(population.chromosomes[i].genes) == 5 for i = 1:60])
    @test all([
        all([chromosome.genes[i] <= 1 for i = 1:length(chromosome.genes)]) for
        chromosome in population.chromosomes
    ])

    population_initialization = RealUniformInitialization(20, 50, (-14.3, 18.2))
    population = population_initialization()
    @test length(population.chromosomes) == 20
    @test all([length(population.chromosomes[i].genes) == 50 for i = 1:20])
    @test all([
        all([
            chromosome.genes[i] <= 18.2 && chromosome.genes[i] >= -14.3 for
            i = 1:length(chromosome.genes)
    ...
    @test_throws ArgumentError RealUniformInitialization(
        10,
        3,
        [(0.0, 1.0), (-10.0, 10.0), (5.0, 5.0)],
    )

    population_initialization = RealUniformInitialization(30, 4, (1, 10))
    population = population_initialization()
    @test length(population.chromosomes) == 30
    @test all([length(population.chromosomes[i].genes) == 4 for i = 1:30])
    @test all([
        all([
            chromosome.genes[i] >= 1 && chromosome.genes[i] <= 10 for
            i = 1:length(chromosome.genes)
        ]) for chromosome in population.chromosomes
    ])

    population_initialization = RealUniformInitialization(
        15,
        6,
    ...
    @test all([length(population.chromosomes[i].genes) == 6 for i = 1:15])
    @test all([
        chromosome.genes[1] >= 1 &&
        chromosome.genes[1] <= 5 &&
        chromosome.genes[2] >= 10 &&
        chromosome.genes[2] <= 20 &&
        chromosome.genes[3] >= 30 &&
        chromosome.genes[3] <= 40 &&
        chromosome.genes[4] >= 50 &&
        chromosome.genes[4] <= 60 &&
        chromosome.genes[5] >= 70 &&
        chromosome.genes[5] <= 80 &&
        chromosome.genes[6] >= 90 &&
        chromosome.genes[6] <= 100 for chromosome in population.chromosomes
    ])
end

@testset "AsymmetricPopulationInitialization" begin
    population_initialization =
        RealUniformInitialization(10, 3, [(0.0, 1.0), (-10.0, 10.0), (5.0, 15.0)])
    population = population_initialization()
    @test length(population.chromosomes) == 10
    @test all([length(population.chromosomes[i].genes) == 3 for i = 1:10])
    @test all([
        chromosome.genes[1] >= 0.0 &&
        chromosome.genes[1] <= 1.0 &&
        chromosome.genes[2] >= -10.0 &&
        chromosome.genes[2] <= 10.0 &&
        chromosome.genes[3] >= 5.0 &&
        chromosome.genes[3] <= 15.0 for chromosome in population.chromosomes
    ])
...
@testset "SudokuInitialization" begin
    ...
    @test_throws ArgumentError SudokuInitialization(
        10,
